# Demo_Stack — TypeScript + Python Reference Platform Stack

A hybrid reference platform stack combining **Python (FastAPI Clean Architecture / DDD)** backend with an **Angular 20 TypeScript SPA** frontend, validated against the **Testable Engineering Strategy Matrix (v0.2)**.
...
```mermaid
flowchart LR
    subgraph UI["Frontend (Angular 20 TypeScript)"]
...
### 2. Run TypeScript Frontend (Angular 20)
```bash
cd frontend
npm install
npm run lint:biome
