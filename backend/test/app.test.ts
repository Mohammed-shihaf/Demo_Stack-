...
describe('TypeScript Unified Backend REST API Tests', () => {
  let app: any;
  ...
  it('POST /api/records creates a new record and returns 201', async () => {
    const res = await request(app)
      .post('/api/records')
      .send({ title: 'Test Record 1', description: 'Enterprise DDD TypeScript' });

    assert.strictEqual(res.status, 201);
    assert.strictEqual(res.body.title, 'Test Record 1');
    assert.strictEqual(res.body.description, 'Enterprise DDD TypeScript');
