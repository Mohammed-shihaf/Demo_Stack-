...
public class DataFlowSampleServiceTests
{
    ...
    [Fact]
    public void DeriveLeaseDiscountTier_Gold()
    {
        var result = DataFlowSampleService.DeriveLeaseDiscountTier(70000m);
        Assert.Equal("gold", result.Tier);
        Assert.Equal(0.15m, result.DiscountRate);
    }

    [Fact]
    public void DeriveLeaseDiscountTier_Silver()
    {
        var result = DataFlowSampleService.DeriveLeaseDiscountTier(30000m);
        Assert.Equal("silver", result.Tier);
        Assert.Equal(0.08m, result.DiscountRate);
    }

    [Fact]
    public void DeriveLeaseDiscountTier_StandardReturnsEarlyWithNoDiscount()
    {
        var result = DataFlowSampleService.DeriveLeaseDiscountTier(5000m);
