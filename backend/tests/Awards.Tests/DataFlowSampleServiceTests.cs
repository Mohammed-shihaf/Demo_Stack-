...
public class DataFlowSampleServiceTests
{
    ...
    [Fact]
    public void DeriveDiscountTier_Gold()
    {
        var result = DataFlowSampleService.DeriveDiscountTier(150000m);
        Assert.Equal("gold", result.Tier);
        Assert.Equal(0.15m, result.DiscountRate);
    }

    [Fact]
    public void DeriveDiscountTier_Silver()
    {
        var result = DataFlowSampleService.DeriveDiscountTier(50000m);
        Assert.Equal("silver", result.Tier);
        Assert.Equal(0.08m, result.DiscountRate);
    }

    [Fact]
    public void DeriveDiscountTier_StandardReturnsEarlyWithNoDiscount()
    {
        var result = DataFlowSampleService.DeriveDiscountTier(5000m);
