/**
 * Data Flow Sample Utility (TypeScript / Frontend)
...
function classifyTier(orderTotal: number): DiscountResult['tier'] {
...
export function deriveDiscountTier(orderTotal: number): DiscountResult {
  const tier = classifyTier(orderTotal); // cross-function definition
  let discountRate = 0;
  if (tier === 'gold') {
    discountRate = 0.15;
  } else if (tier === 'silver') {
...
/** Multiple definitions of `edgeCount` across nested loops -- gives
 * partial-uses-coverage and coverage-reporting-validation checks a
