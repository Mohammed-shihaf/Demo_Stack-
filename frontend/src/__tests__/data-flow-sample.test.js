...
import { accumulateWithEarlyExit, deriveLeaseDiscountTier, buildAdjacencySample } from '../utils/data-flow-sample.js';
...
describe('data-flow-sample utility', () => {
  ...
  it('classifies a gold-tier lease and applies the discount', () => {
    const result = deriveLeaseDiscountTier(70000);
    expect(result.tier).toBe('gold');
    expect(result.discountRate).toBe(0.15);
  });

  it('classifies a silver-tier lease and applies the discount', () => {
    const result = deriveLeaseDiscountTier(30000);
    expect(result.tier).toBe('silver');
    expect(result.discountRate).toBe(0.08);
  });

  it('returns early with no discount for a standard-tier lease', () => {
    const result = deriveLeaseDiscountTier(5000);
