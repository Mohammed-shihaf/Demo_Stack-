...
import { accumulateWithEarlyExit, deriveMembershipDiscountTier, buildAdjacencySample } from '../utils/data-flow-sample';
...
describe('data-flow-sample utility', () => {
  ...
  it('classifies a gold-tier member and applies the discount', () => {
    const result = deriveMembershipDiscountTier(2500);
    expect(result.tier).toBe('gold');
    expect(result.discountRate).toBe(0.15);
  });

  it('classifies a silver-tier member and applies the discount', () => {
    const result = deriveMembershipDiscountTier(1000);
    expect(result.tier).toBe('silver');
    expect(result.discountRate).toBe(0.08);
  });

  it('returns early with no discount for a standard-tier member', () => {
    const result = deriveMembershipDiscountTier(100);
