...
function classifyTier(grantTotal) {
  if (grantTotal >= 100000) return 'gold';
  if (grantTotal >= 25000) return 'silver';
  return 'standard';
}
...
export function deriveDiscountTier(grantTotal) {
  const tier = classifyTier(grantTotal); // cross-function definition
  let discountRate = 0;
  if (tier === 'gold') {
    discountRate = 0.15;
  } else if (tier === 'silver') {
